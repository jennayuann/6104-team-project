          ...
          >
            <defs>
              <marker
                ...
                orient="auto"
              >
                <polygon points="0 0, 10 3, 0 6" fill="#415a77" />
              </marker>
            </defs>
            <!-- Edges -->
            <g id="edges-group">
          ...
          <div class="nodes-container">
            ...
            >
              ...
              <div class="node-tooltip">
                ...
                <div v-if="node.username && node.username !== node.label" class="tooltip-username">
                  {{ node.username }}
                </div>
              </div>
            </div>
...
function calculateLayout(): { nodes: Node[]; edges: Edge[] } {
  ...
  allNodeIds.forEach(nodeId => {
    ...
    const label = profileData.profile?.headline || profileData.username || nodeId;
  ...
  () => {
    ...
    if (layoutedNodes.value.length > 0) {
      layoutedNodes.value.forEach(node => {
        ...
        node.label = profileData.profile?.headline || profileData.username || node.id;
